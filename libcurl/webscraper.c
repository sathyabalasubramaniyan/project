...
int main(int argc, char **argv)
{
	...
	webpage_to_text("https://www.restapitutorial.com/",path);
...
void readfile(char *path1)
{
        char c,c1;
        char word[1000];
        char *path2="/home/user/Desktop/newfile7.txt";
        char *path3="/home/user/Desktop/result1.txt";
      ...
      {
       c=fgetc(fptr1);//next character
       while(c!=';'&&c!='c'&&c!=' ')//break if it is &copy or one symbol(&)
       {
        c=fgetc(fptr1);
       }
       if(c=='c')//loop until it get y for &copy (symbol)
       {
        while(c!='y')
        {
         c=fgetc(fptr1);
     ...
     while(fscanf(f, "%s", word) != EOF)//for removing extra spaces 
     {
      ...
      else if((!strcmp(word,"."))||(!strcmp(word,"00"))||(!strcmp(word,":"))||(!strcmp(word,"₹"))||(!strcmp(word,"..")))//removing invalid symbols
      {
      }
      else
      {
...
void occurences(char *path)
{
    ...
    char *path4="/home/user/Desktop/result2.txt";
